...
const AddFreight = ({ toggleShowComponent, applyChanges, itemData, index }) => {
  ...
  const calcPrice = useDebouncedCallback((items) => {
    const price = Math.round(Number(items) * 166.7);
    dispatchDetail({
      type: "price_per_kg".toUpperCase(),
      value: { items: items, total_price: price },
    });
  }, 200);

  useEffect(() => {
    const { items } = data.price_per_kg;
    calcPrice(items);
  }, [data.price_per_kg, calcPrice]);
  ...
  return (
    <div className={classes["form"]}>
      ...
      <div className={classes["price-per-kg"]}>
